...
function getPlainTextFromHtml(html) {
  ...
  var elementsToRemove = temp.querySelectorAll(
    "style, script, noscript, iframe, object, embed, span.mw-editsection, img"
  );
...
function findSectionsMinervaInTalkPage() {
  headingElements = document.querySelectorAll(
    "#mw-content-text > .mw-parser-output > .mw-heading"
  );
...
function findSectionsMinervaInArticlePage() {
  headingElements = document.querySelectorAll(
    "#mw-content-text > .mw-parser-output > h2.section-heading"
  );
  if (headingElements.length === 0) {
    console.error("can not find heading elements");
    return null;
